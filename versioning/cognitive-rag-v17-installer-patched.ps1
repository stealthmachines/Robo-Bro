...
#   [FIX-14] brain.py: _unload_embed_model() frees nomic-embed-text VRAM
#            immediately after every ChromaDB op (prevents conflict with
#            cograg-gpu on shared 12 GB VRAM)
#   [FIX-15] engine.py: _ollama_sem Semaphore(1) serialises all Ollama
#            calls; background_loop skips if semaphore locked; timeout
#            raised to 300s; probe interval raised to 300s (5 min)
#   [FIX-16] engine.py + api.py + extension.js: think:false + keep_alive
#            "30m" on every Ollama call; prevents invisible <think> output
#            and model unloading between requests
#   [FIX-17] Modelfile.gpu: num_gpu removed (wu-wei auto-plan); num_ctx
#            reduced to 4096; default base changed to qwen3:latest (8B)
#            because WDDM on Windows prevents cudaMalloc for the 30B MoE
#            model even with free VRAM reported. After a system reboot the
#            30B model (qwen3.6:latest) can be restored.
...
#            memory_consolidation, self_diagnosis, code_health_check,
#            embedding_maintenance, cleanup_old_data.
#            /maintenance endpoint added to expose recent maintenance events.
# ================================================================
...
# ── Configure Ollama GPU layers via Modelfile (FIX-17) ──────────────────────
# FIX-17: num_gpu removed — Ollama auto-plans GPU/CPU split (wu-wei).
# On Windows WDDM the display driver locks physical VRAM pages that
# nvidia-smi reports as free but cudaMalloc cannot use. Prefer a model
# that fits comfortably: qwen3:latest (8B, 5.2 GB). To restore 30B MoE
# (qwen3.6:latest), reboot first, then change FROM and rebuild.
Step "Configuring Modelfile (FIX-17: wu-wei GPU auto-plan)"
# Select base model: prefer qwen3:latest (8B) for WDDM compatibility
$WuWeiBase = $MODEL  # installer already picked best available model
if ($WuWeiBase -match "qwen3.6|30b|moe") {
    Warn "Note: $WuWeiBase is a 30B MoE model. Will load fine after reboot; may fail on first session due to WDDM VRAM fragmentation."
}
$modelfileContent = @"
FROM $WuWeiBase
...
def _unload_embed_model():
    """FIX-14: Release nomic-embed-text VRAM immediately after use."""
    try:
        body = json.dumps({"model": "nomic-embed-text", "keep_alive": 0, "prompt": ""}).encode()
    ...
    if text and len(text) > 10:
        try:
            ...
            if c:
                ...
                _unload_embed_model()  # FIX-14: free VRAM immediately after embedding
        except Exception:
            pass
...
# FIX-15: Limit Ollama to one concurrent request
...
FIX-19: /query is now agentic -- detects web/diagnosis/docs intent, runs real tools,
        injects grounded results into LLM context before generating the answer.
"""
...
async def query(req: Q):
    ...
    system = (
        ...
        "2. If [OCR: ...] blocks are present: quote them VERBATIM — copy the exact text character for character.\n"
