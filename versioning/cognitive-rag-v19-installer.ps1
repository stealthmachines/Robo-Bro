...
Note: SearXNG requires a running local instance (see searxng-compose.yml).
"""
...
def _fetch_url(url: str, timeout=10) -> str:
    """Fetch a URL, trying requests (better TLS/redirect handling) then urllib fallback.
    Also attempts Google webcache if direct fetch fails."""
    candidates = [url]
    # Add Google webcache as fallback for bot-protected sites
    domain_only = re.sub(r"https?://", "", url).rstrip("/")
    candidates.append(f"https://webcache.googleusercontent.com/search?q=cache:{url}")
    # Also try Bing cache
    candidates.append(f"https://cc.bingj.com/cache.aspx?q={urllib.parse.quote(domain_only)}&url={urllib.parse.quote(url)}")
    ...
    for attempt_url in candidates:
        # Try with requests first (better TLS negotiation)
        try:
...
def _bing_scrape(query: str, n=5) -> list:
    """Scrape Bing search results page — works when APIs return nothing."""
    try:
        import requests as _req
        ...
        if r.status_code != 200:
            return []
        # Extract titles + URLs + snippets from Bing result HTML
        results = []
        # Title + URL
        title_blocks = re.findall(
            r'<h2[^>]*><a[^>]+href="([^"]+)"[^>]*>([^<]+)</a>', r.text)
        # Snippets
        snippet_blocks = re.findall(
            r'<p class="b_lineclamp\d[^"]*">(.+?)</p>', r.text, re.DOTALL)
    ...
    if fetch_pages and results:
        for r in results[:2]:
            ...
            if u and u.startswith("http"):
                ...
                if text:
                    pages[u] = text
    # Fallback: if no results at all, try fetching any URL/domain mentioned in query directly
    if not results:
        for url in _extract_urls(query):
            text = _fetch_url(url)
            if text:
                results.append({"title": url, "url": url, "snippet": text[:600]})
                pages[url] = text
                source = "direct_fetch"
                break
...
async def query(req: Q):
    ...
    # ── Intent: web search ────────────────────────────────────────────────────
    if any(t in q_lower for t in _WEB_TRIGGERS):
        try:
            raw = await asyncio.to_thread(web_search, req.query, True)
    ...
    try:
        chunks = hybrid_retrieve(req.query, top_k=4)
        if chunks:
            doc_ctx = "\n".join(
    ...
    system = (
        ...
        "live web search and direct URL fetching. Tools are executed server-side before this "
        "prompt — results appear below as [Web search results], [Relevant docs], etc.\n\n"
        "STRICT RULES:\n"
        "1. NEVER say you cannot access the internet, visit URLs, or search the web.\n"
        "2. NEVER say 'no context was provided' about a website — the system fetches it "
        "automatically when a URL is in the query.\n"
        "3. If [Web search results] are present: summarise the actual content found.\n"
        "4. If the web results say 'no content retrieved': explain the site was unreachable "
        "(bot protection / JS required / timeout) — not that you lack access.\n"
        "5. If web results show mostly image references: explain the post's content is "
        "primarily images and quote whatever text was extractable.\n"
        "6. If [Relevant docs] are present: cite them.\n"
        "7. If [Self-diagnosis results] are present: report the actual numbers.\n\n"
        f"Web search status for THIS call: {web_status}"
    )
    user_msg = (
        f"Query: {req.query}\n\n"
        f"Memory context:\n{mem_ctx}"
